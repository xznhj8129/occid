...
OCCID models have exactly two authored semantic roles:
...
`Concept` and `Representation` are explicit authored model roles. There is no `Type` semantic role, authored or compiler-derived. Runtime field usability is not an ontological category.

Every authored model remains a nominal compiled runtime model. The semantic hierarchy is preserved as metadata while generated target-language classes remain structurally flat.
...
`parent` is the single authoritative semantic-tree edge. If model `B` declares `parent: A`, then `B is_a A` and inherits `A` fields where field inheritance is applicable. There is no second authored child list, no `variants` list, and no alternate family graph.

Compilation resolves inherited fields, preserves every model by name, preserves `parent`, and derives each model's direct `children` by reversing the parent edges. Runtime classes are flat; semantic compatibility is answered by the compiled parent/children registry.

Runtime `model_id` values identify concrete compiled Concepts and Representations on the compact wire. A model ID does not establish semantic ancestry.
...
Unknown model keys are errors. Authored `children`, `variants`, and `family` are not model keys.
...
```text
Concept
    A semantic category whose distinction belongs in the ontology. It may have
    fields and may be used directly as a runtime value or field reference.

Representation
    An explicit concrete data-bearing form. It may be record-shaped (`fields`)
    or atomic (`type`). A Representation is still a node in the same semantic
    tree; its parent relation has exactly the same is-a meaning.

Vocabulary
    A closed controlled value set represented by an enum. Vocabulary is
    identified by declaration kind and has no model `semantic_role`.
```

There is no `Type` role. Leafness, having fields, being referenced by another field, or being emitted by a runtime generator never changes a model's semantic role.
...
`TaskInformation` is a Task because it declares `parent: Task`. No second declaration is permitted or required at `Task`.
...
- `models`: every authored Concept and Representation in one model namespace;
- `vocabulary`: enums;
- `maps`: named constant maps.

For every compiled model:

- `semantic_role` preserves whether the authored model is a `concept` or `representation`;
- `parent` preserves the authored semantic parent when one exists;
- `children` is the direct reverse index derived from authored `parent` edges;
- record fields are the flattened effective inherited fields;
- no `family` projection is generated;
- no semantic node is removed because it has children.

Generated target-language classes are flat projections. A generated child class does not inherit its semantic parent's Python class. Instead each generated model exposes semantic parent/children metadata and the runtime `is_a()` relation follows that registry.

Named model references remain nominal during compilation. The compiler does not rewrite a parent reference into a union of currently known descendants. For example:

```yaml
condition: optional Condition
```

remains `Condition` in `occid.yaml` no matter how many Condition children exist. The Python runtime projects model references through `Semantic[Condition]`, which accepts an already-materialized flat descendant when `is_a(descendant, Condition)` is true.

Explicit unions written by the author remain explicit unions. Only compiler-synthesized descendant unions are forbidden.

Each compiled Representation contains either `fields` or `type`, never both. A compiled atomic Representation therefore remains visibly atomic:
...
There is no reverse child declaration in authored schema. The compiler discovers direct children by reading every model's `parent`. A model cannot declare a parent while being omitted from that parent's children, because there is no second membership graph to disagree.
...
- Every parent edge means semantic is-a.
- A child inherits all parent fields into its effective compiled shape.
- Concept and Representation children use the same parent semantics.
- An atomic Representation may not declare or inherit record fields.
- An atomic Representation may not be used as a parent; its value shape is complete.
- `children` is compiler-derived direct metadata and may not be authored.
- `variants` and `family` are not semantic hierarchy mechanisms.
...
### 13.4 Extending an existing hierarchy

A module extends an existing hierarchy by declaring an ordinary model whose `parent` is already available:
...
Every compiled Concept and Representation has a numeric OCCID model ID generated as part of the compiled contract.
...
- authored Concept/Representation role;
- authored parentage;
...
### 15.2 Compiled semantic metadata

Every generated runtime model exposes:

```text
__occid_semantic_role__   concept or representation
__occid_parent__          direct semantic parent or None
__occid_children__        compiler-derived direct semantic children
```

The runtime also exposes the parent/children registry and `is_a()`. These describe semantic compatibility only. They do not create target-language inheritance and must not be treated as application dispatch tables.
...
- An atomic Representation used through a semantic field whose declared model exactly matches the concrete value is serialized as its underlying value directly; a semantic descendant carries its model ID so the concrete form can be recovered.
...
- a nested model that does not satisfy the field's declared semantic model through `is_a()`;
- a top-level model ID inconsistent with a specifically requested concrete class.

`model_id` identifies the concrete runtime model. Compatibility with a field's declared semantic model is determined by `is_a()` over the compiled parent graph, not by Python inheritance, generated unions, or the numeric ID itself.
...
The authored semantic graph is defined only by `parent`. Authored model role is `Concept` or `Representation`, Vocabulary is defined by enums, and there is no separate authoritative ontology file. The compiler preserves parent edges and derives direct child relationships in `occid.yaml`.
