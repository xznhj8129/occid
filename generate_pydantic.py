...
DEFAULT_OUTPUT_DIR = SCRIPT_DIR / "liboccid" / "py"
TEMPLATE_DIR = SCRIPT_DIR / "lib" / "templates" / "pydantic"
CORE_SCHEMA_MAX_PARTS = 3
ONTOLOGY_OUTPUT = SCRIPT_DIR / "ontology.yaml"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--ontology-output", type=Path, default=ONTOLOGY_OUTPUT)
...
def render_ontology(modules: list[ModuleDef]) -> str:
    models = [model for module in modules for model in module.models]
    concepts = {model.name: model for model in models if model.semantic_role == "concept"}
    children: dict[str, list[str]] = {name: [] for name in concepts}
    roots: list[str] = []

    for model in models:
        if model.name not in concepts:
            continue
        if model.parent is None:
            roots.append(model.name)
        elif model.parent not in concepts:
            raise SchemaError(
                f"Concept {model.name} has non-Concept parent {model.parent}; cannot render ontology tree"
            )
        else:
            children[model.parent].append(model.name)

    lines = ["# GENERATED by generate_pydantic.py. Do not edit.", ""]

    def append_node(name: str, indent: int) -> None:
        model = concepts[name]
        prefix = " " * indent
        if not model.description and not children[name]:
            ...
            if model.description:
                ...
                description_prefix = " " * (indent + 2)
                lines.extend(f"{description_prefix}{line}" for line in description.splitlines())
            if children[name]:
                lines.append(f"{prefix}  children:\n")
                for child in children[name]:
                    append_node(child, indent + 4)
        lines.append("")
...
def main() -> None:
    ...
    args.ontology_output.write_text(render_ontology(modules))
    print(f"input={args.input}")
    print(f"output_dir={args.output_dir}")
    print(f"ontology_output={args.ontology_output}")
