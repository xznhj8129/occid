...
def parse_model(name: str, spec: dict) -> ModelDef:
    return ModelDef(
        ...
        fields=[parse_field(field_name, field_spec) for field_name, field_spec in spec["fields"].items()],
    )
    ...
    for path in sorted(schema_dir.glob("*.schema.yaml")):
        ...
        modules.append(
            ModuleDef(
                ...
                mappings=[parse_mapping(name, spec) for name, spec in (data.get("mappings") or {}).items()],
            )
        )
    return modules
...
def validate_schema(modules: list[ModuleDef], symbol_index: dict[str, str], enum_members: dict[str, set[str]]) -> None:
    for module in modules:
        for mapping_def in module.mappings:
        ...
        for model_def in module.models:
            ...
            for field_def in model_def.fields:
                ...
                if field_def.const and field_def.default is None:
...
def render_common_module(module: ModuleDef, enum_members: dict[str, set[str]]) -> str:
    sections = [load_template("common_header.py"), load_template("common_intenum.py")]
    if module.enums:
        sections.append("\n\n".join(render_enum_block(enum_def) for enum_def in module.enums))
    if module.mappings:
        sections.append("### Mappings")
        sections.append("\n\n".join(render_mapping_block(mapping_def, enum_members) for mapping_def in module.mappings))
    sections.append(load_template("common_runtime.py"))
    if module.models:
...
def render_module(module: ModuleDef, symbol_index: dict[str, str], enum_members: dict[str, set[str]]) -> str:
    if module.name == "common":
        return render_common_module(module, enum_members)
...
def render_init(module_names: list[str]) -> str:
    lines = [f"from .{name} import *" for name in module_names]
    lines.extend(
        [
            "",
